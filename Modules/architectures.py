...
class GraphonGNN(nn.Module):
    ...
    def __init__(self,
        ...
        for l in range(self.L):
            ...
            #\\ Pooling
            gfl.append(self.rho(self.SelectedIdx[l], self.SelectedIdx[l+1]))
        # And now feed them into the sequential
        self.GFL = gml.mySequential(*gfl) # Graph Filtering Layers
    ...
    def forward(self, x, S_la):
        ...
        x = x.to(torch.double)
        S_la = S_la.to(torch.double)
        thisDevice = x.device
        B = S_la.shape[0]
        E = S_la.shape[1]
        self.S_seq = [S_la]
        for i in range(1, len(self.SelectedIdx)):
            idx_temp = self.SelectedIdx[i]
            if self.Integration:
                S_temp = torch.zeros([B, E, len(idx_temp), len(idx_temp)])
                for ii in range(len(idx_temp)):
                    for jj in range(ii, len(idx_temp)):
                        if jj == len(idx_temp) -1 and ii == len(idx_temp) -1:
                            temp_ave = torch.mean( S_la[ :, : , idx_temp[ii] : -1, idx_temp[jj]: -1], dim =(2,3) )
                        elif ii == len(idx_temp) -1:
                            temp_ave = torch.mean( S_la[:, :, idx_temp[ii]:-1, idx_temp[jj]: idx_temp[jj+1]], dim =(2, 3) )
                        elif jj ==len(idx_temp) -1:
                            temp_ave = torch.mean( S_la[:, :, idx_temp[ii]:idx_temp[ii+1], idx_temp[jj]: -1], dim =(2, 3) )
                        else:
                            temp_ave = torch.mean( S_la[:, :, idx_temp[ii]:idx_temp[ii+1], idx_temp[jj]: idx_temp[jj+1]], dim =(2, 3) )
                        S_temp[:,:, ii, jj]= temp_ave
                        S_temp[:,:,jj,ii] = S_temp[:,:, ii,jj]
                S_temp.to(thisDevice)
                self.S_seq.append(S_temp)
                # print(S_temp)
            else:
                S_temp = S_la[:, :, :, idx_temp]
                S_temp = S_temp[:, :, idx_temp, :]
                S_temp.to(thisDevice)
                self.S_seq.append(S_temp)

        
                       
        y, _ = self.GFL(x, self.S_seq)
        # Flatten the output
...
class SelectionGNN(nn.Module):
    ...
    def __init__(self,
        ...
        self.K = nFilterTaps # Filter taps
        self.E = 1 # Number of edge features
        self.N = [num_points] + nSelectedNodes
        ...
        # And now feed them into the sequential
        if self.coarsening:
            self.GFL = gml.mySequential(*gfl)
        else:
            self.GFL = gml.mySelSequential(*gfl) # Graph Filtering Layers
        #\\\ MLP (Fully Connected Layers) \\\
        fc = []
        if len(self.dimLayersMLP) > 0: # Maybe we don't want to MLP anything
            # The first layer has to connect whatever was left of the graph
    ...
    def splitForward(self, x, S):
        ...
        if self.coarsening:
            ...
            for i in range(S.shape[0]):
                S_this = S[i, 0, :, :]
                S_this = scipy.sparse.csr_matrix(S_this)
                S_temp, self.order = Utils.graphTools.coarsen(S_this, levels=self.L,
                                                       self_connections=False)
                for j in range(len(S_temp)):
                    S_temp[j] = S_temp[j].todense().A.reshape([1, self.E, S_temp[j].shape[0], S_temp[j].shape[1]])
                    S_temp[j] = torch.tensor(S_temp[j]).to(thisDevice)
          

                    if i == 0:
                        S_seq.append(S_temp[j])
                    else:
                        S_seq[j] = torch.cat((S_seq[j], S_temp[j]), 0)

            # Now, GSO is a list of csr_matrix with self.L+1 coarsened GSOs,
            # we need to torch.tensor them and put them in a list.
            # order is just a list of indices to reorder the nodes

            y, _ = self. GFL(x, S_seq)
        else:
            y,_ = self.GFL(x, S)
        # Flatten the output
        yFlat = y.reshape(batchSize, self.F[-1] * self.N[-1])
