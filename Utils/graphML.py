...
class mySequential(nn.Module):
    ...
    def __getitem__(self, idx) -> Union['mySequential', T]:
    ...
    def forward(self, input, S):
        ...
        S_in = S[0]
        # print(input.device)
        thisDevice = input.device
        for module in self:
            # print('count %d' % count)
            if count % 3 == 0:
                S_in = S[int(np.floor(count/3))]

            input, S_in = module(input, S_in)
            input.to(thisDevice)
            S_in.to(thisDevice)
            count = count + 1
        return input, S_in
...
class mySelSequential(nn.Module):
    """A sequential container.
    Modules will be added to it in the order they are passed in the
    constructor. Alternatively, an ``OrderedDict`` of modules can be
    passed in. The ``forward()`` method of ``Sequential`` accepts any
    ...
    def __init__(self, *args):
        super(mySelSequential, self).__init__()
        if len(args) == 1 and isinstance(args[0], OrderedDict):
            for key, module in args[0].items():
                self.add_module(key, module)
        else:
    ...
    def __getitem__(self, idx) -> Union['mySelSequential', T]:
        if isinstance(idx, slice):
            return self.__class__(OrderedDict(list(self._modules.items())[idx]))
        else:
            return self._get_item_by_idx(self._modules.values(), idx)
    ...
    def __dir__(self):
        keys = super(mySelSequential, self).__dir__()
        keys = [key for key in keys if not key.isdigit()]
        return keys
...
class maxpool1dwithS(nn.Module):
    """
    NoActivation creates an activation layer that does nothing
        It is for completeness, to be able to switch between linear models
        and nonlinear models, without altering the entire architecture model
    ...
    def forward(self, x, S):
        ...
        m = nn.MaxPool1d(self.kernel)
        y = m(x)
        
        return y, S
    
    def extra_repr(self):
        reprString = "maxpool 1d with S"
        return reprString
...
class GraphonCoarsening(nn.Module):
    ...
    def __init__(self, InputIdxes, OutputIdxes):

        super().__init__()
        self.InputIdxes = InputIdxes
        self.OutputIdxes = OutputIdxes


    def forward(self, x, S):
        assert x.shape[2] == len(self.InputIdxes)
        thisDevice = x.device
        B = x.shape[0]
        F = x.shape[1]
        y = torch.empty(B, F, len(self.OutputIdxes))
        for i in range(len(self.OutputIdxes)):
            for j in range(len(self.InputIdxes)):
                if j == 0:
                    if (self.OutputIdxes[i] >= 0) and (self.OutputIdxes[i] < self.InputIdxes[j]):
                        y[:,:,i] = x[:,:,j]
                elif j < len(self.InputIdxes) - 1:
                     if self.OutputIdxes[i] >= self.InputIdxes[j-1] and self.OutputIdxes[i] < self.InputIdxes[j]:
                        y[:,:,i] = x[:,:,j]
                else:
                     if self.OutputIdxes[i] >= self.InputIdxes[j-1] and self.OutputIdxes[i] <+ self.InputIdxes[j]:
                        y[:,:,i] = x[:,:,j]
...
class MaxPoolLocal(nn.Module):
    ...
    def forward(self, x, S):
        ...
        # S is of shape B x E x N x N (And I don't care about E, because the
        # computeNeighborhood function takes care of it)
        self.N = S.shape[2]
        ...
        for i in range(S.shape[0]):
            neighborhood.append( graphTools.computeNeighborhood(S[i, :, :, :], self.nHops,
                                                      self.nOutputNodes,
                                                      self.nInputNodes,'matrix'))
        ...
        neighborhood = torch.tensor(neighborhood).to(device)
        # The neighborhood matrix has to be a tensor of shape
        #   B x nOutputNodes x maxNeighborhoodSize
        assert neighborhood.shape[1] == self.nOutputNodes and neighborhood.shape[0] == batchSize
        assert neighborhood.max() <= self.nInputNodes
        # Store all the relevant information
        self.maxNeighborhoodSize = neighborhood.shape[2]
        self.neighborhood = neighborhood

        x = x.unsqueeze(3) # B x F x N x 1
        x = x.repeat([1, 1, 1, self.maxNeighborhoodSize]) # B x F x N x maxNeighbor
        # And the neighbors that we need to gather are the same across the batch
        # and feature dimensions, so we need to repeat the matrix along those
        # dimensions
        gatherNeighbor = self.neighborhood.reshape([batchSize, 1,
                                                    self.nOutputNodes,
                                                    self.maxNeighborhoodSize])
        gatherNeighbor = gatherNeighbor.repeat([1, dimNodeSignals, 1,1])
        # And finally we're in position of getting all the neighbors in line
...
class GraphFilter(nn.Module):
    ...
    def forward(self, x, S):
        ...
        B = x.shape[0]
        F = x.shape[1]
        # print('x device %s' % x.device)
        # print('S device %s' % S.device)
        thisDevice = x.device
        S = S.to(thisDevice)
        Nin = x.shape[2]
        self.N = S.shape[2]
